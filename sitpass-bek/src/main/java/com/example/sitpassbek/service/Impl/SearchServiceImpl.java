    ...
    private Query buildSimpleSearchQuery(List<String> tokens, Map<String, RangeDTO> ranges) {
        ...
        return BoolQuery.of(q -> {
            ...
            q.must(mb -> mb.bool(b -> {
                ...
                ranges.forEach((field, range) -> b.filter(fb -> {

                    RangeQuery rangeQuery = RangeQuery.of(r -> r
                            .date(d -> d
                                    .field(field)
                                    .gte(range.min() != null ? JsonData.of(range.min()).toString() : null)
                                    .lte(range.max() != null ? JsonData.of(range.max()).toString() : null)
                            )
                    );
                    return fb.range(rangeQuery);
                }));
    ...
    private Query buildAdvancedSearchQuery(List<String> expression, Map<String, RangeDTO> ranges) {
        ...
        return BoolQuery.of(b -> {
            ...
            for (Map.Entry<String, RangeDTO> entry : ranges.entrySet()) {
                ...
                b.filter(f -> {
//                    RangeQuery.Builder rangeQuery2 = new RangeQuery.Builder().field(field);
//                    if (range.min() != null) rangeQuery2.gte(JsonData.of(range.min()));
//                    if (range.max() != null) rangeQuery2.lte(JsonData.of(range.max()));
                    RangeQuery rangeQuery = RangeQuery.of(r -> r
                            .date(d -> d
                                    .field(field)
                                    .gte(range.min() != null ? JsonData.of(range.min()).toString() : null)
